...
function count(list2, predicate) {
  return fold(
    list2,
    0,
    (acc, value3) => {
...
function createElementNode({ prev, next, dispatch, stack }) {
  ...
  for (const attr of next.attrs) {
    ...
    const value3 = attr[1];
    if (attr.as_property) {
      if (el[name2] !== value3) el[name2] = value3;
      ...
      if (el.getAttribute(name2) !== value3) el.setAttribute(name2, value3);
      if (name2 === "value" || name2 === "selected") el[name2] = value3;
      if (canMorph) prevAttributes.delete(name2);
    }
  }
  if (className !== null) {
...
// build/dev/javascript/mooquiz/app.ffi.mjs
function get_localstorage(key) {
  const json = window.localStorage.getItem(key);
  if (json === null) return new Error(void 0);
  try {
...
var Question = class extends CustomType {
  constructor(id, text3, answers, correct, selected) {
    super();
    this.id = id;
    this.text = text3;
...
var QuizResult = class extends CustomType {
  constructor(answers, score, out_of) {
    super();
    this.answers = answers;
    this.score = score;
    this.out_of = out_of;
...
function encode_result(result) {
  return object2(
    toList([
      ["shareString", array2(result.answers, bool)],
      ["score", int2(result.score)],
      ["outOf", int2(result.out_of)]
    ])
  );
}
...
function get_today() {
  return from(
    (dispatch) => {
      echo("Getting today's attempt", "src/mooquiz.gleam", 152);
      let $ = get_localstorage(date_format());
      if ($.isOk()) {
        let result = $[0];
        echo(result, "src/mooquiz.gleam", 155);
        dispatch(new ReadAnswers(result));
        return void 0;
      } else {
        echo("Fuck", "src/mooquiz.gleam", 160);
        return void 0;
      }
    }
  );
...
function unanswered_questions(model) {
  return any(
    model.questions,
    (q) => {
      return isEqual(q.selected, new None());
    }
  );
}
function calculate_results(questions) {
  let score = count(
    questions,
    (q) => {
      return isEqual(q.selected, new Some(q.correct));
    }
  );
...
function result_panel(model) {
  ...
  return div(
    ...
    toList([
      ...
      div(toList([]), toList([text2(share_string(result.answers))]))
    ])
  );
}
...
function answer_radio(question, answer, submitted) {
  ...
  if (submitted && $1) {
    ...
    return input(
      toList([
        type_("radio"),
        name("question-" + to_string(question.id)),
        value(
          to_string(question.id) + "-" + to_string(answer.pos)
        ),
        on_input((var0) => {
          return new SelectAnswer(var0);
        })
      ])
    );
  }
}
function answer_div(answer, question, submitted) {
  let bg = (() => {
    let $ = isEqual(question.selected, new Some(answer.pos));
    let $1 = question.correct === answer.pos;
    if (!submitted && $) {
  ...
  return label(
    toList([class$("block w-full flex duration-200 p-2 " + bg)]),
    toList([
      span(
        toList([class$("grow")]),
        toList([text2(answer.text)])
      ),
                  ...
                  toList([
                    ...
                    div(
                      ...
                      map(
                        q.answers,
                        (answer) => {
                          return answer_div(answer, q, model.submitted);
                        }
                      )
                    )
                  ])
